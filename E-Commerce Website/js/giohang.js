                ...
                for (var g of list) {
                    if (p.MaSP == g.masp) {
                    	if(p.SoLuong >= g.soLuong) { // check đủ hàng
                    		...
                    		setListInCart(list); // cập nhật localstorage
            				animateCartNumber();
...
function addProductToTable(listProduct) {
    ...
    for (var i = 0; i < listProduct.length; i++) {
        ...
        s += `
			<tr>
				...
				<td class="soluong" >
					<button onclick="decreaseQuantity('` + masp + `')"><i class="fa fa-minus"></i></button>
					<input size="1" onchange="updateQuantityFromInput(this, '` + masp + `')" value=` + soluongSp + `>
					<button onclick="increaseQuantity('` + masp + `')"><i class="fa fa-plus"></i></button>
				</td>
				<td class="alignRight">` + numToString(thanhtien) + ` ₫</td>
				<td class="noPadding"> 
					<i class="fa fa-trash" onclick="deleteProductInCart(` + masp + ",'" + p.TenSP + `')"></i> 
				</td>
			</tr>
		`;
    ...
    s += `
			...
			<tr>
				<td colspan="5">
					<button class="btn btn-primary" data-toggle="modal" data-target="#exampleModal" onclick="makePayment()">
						<i class="fa fa-usd"></i> Thanh Toán 
					</button> 
					<button class="btn btn-danger" onclick="deleteAllProductInCart()">
...
function deleteProductInCart(masp, tensp) {
...
function updateQuantityFromInput(inp, masp) {
...
function updateAll(list) { // Mọi thứ
    animateCartNumber();
